...
class BaseDemucs(nn.Module):
        ...
        for index in range(layers):
            ...
            decode += [
                nn.Conv1d(
                    ...
                    out_channels=hidden_channels * channel_scale,
                    kernel_size=1,
                    stride=1,
                ),
                nn.GLU(dim=1),
                nn.ConvTranspose1d(
                    hidden_channels, decoder_target_out, kernel_size, stride
                ),
            ]
            if index < layers - 1:
    ...
    def forward(self, mix: torch.Tensor):
        ...
        if self.resample > 1:
            for idx in range(0, self.resample // 2):
                sample = F.interpolate(
                    sample, scale_factor=0.5, mode="linear", align_corners=False
                )
...
class ProcessRealTime:
    def __init__(
        ...
        base_model: BaseDemucs = BaseDemucs,
        device: str = "cpu",
    ):
