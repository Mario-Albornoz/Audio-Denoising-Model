...
def generate_dataset():
    ...
    pure_noise_files = [
        f for ext in NOISE_EXTENSIONS for f in NOISE_PATH.rglob(f"*{ext}")
    ]

    if not clean_files:
        raise ValueError(
            f"{len(clean_files)} files in clean files, check path {str(RAW_PATH)}"
        )
    if not pure_noise_files:
        raise ValueError(
            f"{len(pure_noise_files)} files in clean files, check path {str(NOISE_PATH)}"
        )
...
def create_noise_file(clean_file, pure_noise_file) -> None:
    noise, clean = normalize_tensors(
        str(clean_file), pure_noise_file, TARGET_SR, SEGMENT_LENGTH
    )
...
def split_dataset(size_training_dataset: int) -> None:
    ...
    for clean_file, noisy_file in zip(
        train_clean_files[-test_size:], train_noisy_files[-test_size:]
    ):
