...
def analyze_dataset_overview(
    train_clean_path, train_noisy_path, test_clean_path, test_noisy_path
):
    """
    ...
    for name, path in paths.items():
        ...
        stats[name] = {"count": len(files), "files": files}
    ...
    for name, path in paths.items():
        for f in tqdm(
            stats[name]["files"][:50], desc=f"Sampling {name}", leave=False
        ):  # Sample first 50
            try:
    ...
    if not df_durations.empty:
        ...
        ax.set_title(
            "Audio Duration Distribution (Sample)", fontsize=14, fontweight="bold"
        )
    ...
    for name in categories:
        ...
        if name in df_durations["Category"].values:
            avg_dur = df_durations[df_durations["Category"] == name][
                "Duration (s)"
            ].mean()
...
def analyze_audio_quality(train_clean_path, train_noisy_path, num_samples=30):
    ...
    fig.suptitle(
        "Audio Quality Comparison: Clean vs Noisy", fontsize=16, fontweight="bold"
    )
    ...
    if min_len > 0:
        snr_estimate = 20 * np.log10(
            clean_peak[:min_len] / (noisy_peak[:min_len] - clean_peak[:min_len] + 1e-10)
        )
        ...
        ax.axvline(
            np.mean(snr_estimate),
            color="red",
            linestyle="--",
            linewidth=2,
            label=f"Mean: {np.mean(snr_estimate):.1f} dB",
        )
        ax.legend()
        ax.grid(axis="y", alpha=0.3)
...
def plot_waveform_comparison(clean_file, noisy_file, duration=5.0):
    ...
    fig.suptitle(
        f"Waveform Analysis: {Path(clean_file).stem}", fontsize=16, fontweight="bold"
    )
    ...
    ax.plot(
        freqs,
        20 * np.log10(np.abs(clean_fft) + 1e-10),
        label="Clean",
        color="#3498db",
        alpha=0.7,
        linewidth=1.5,
    ...
    ax.plot(
        freqs,
        20 * np.log10(np.abs(noisy_fft) + 1e-10),
        label="Noisy",
        color="#e74c3c",
        alpha=0.7,
        linewidth=1.5,
...
def generate_all_visualizations(
    ...
    if clean_files:
        ...
        if noisy_file.exists():
            ...
            fig3.savefig(
                output_path / "waveform_comparison.png", dpi=150, bbox_inches="tight"
            )
